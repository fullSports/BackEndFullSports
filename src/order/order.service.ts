...
export class OrderService {
  ...
  async ListOrders(): Promise<Order[]> {
    const listOrders = await this.OrderModel.find()
      .populate("cliente")
      .populate("produto")
      .exec();
  ...
  async RegisterOrder(createOrder: Order) {
    ...
    if (!RegisterOrder) throw new NotFoundException();
      ...
      const findByIdProduct = await this.ProductService.searchProductId(
        product
      );
      const findByIdUser = await this.UserModel.findById({ _id: user }).exec();
      if (!findByIdProduct)
        return response.status(404).send({ messagem: "produto não existe" });
      else if (!findByIdUser)
        return response.status(404).send({ messagem: "cliente não existe" });
      else {
        const obj = Object.keys(findByIdProduct.categoriaProduto)[0].toString();
        const ObjUpdate = Object.keys(
          findByIdProduct.categoriaProduto
        )[0].toString();
        const newProduct = {
          categoriaProduto: {
            [ObjUpdate]: {
              ...
              quantidate:
                findByIdProduct.categoriaProduto[obj].quantidade - quantidade,
              imagemProduto:
        ...
        if (!updateNewProduct) throw new NotFoundException();
          ...
          if (!createOrder) throw new NotFoundException();
            ...
            createOrder["total"] =
              findByIdProduct.categoriaProduto[obj].preco * quantidade;
            return createOrder;
          }
        }
      }
  ...
  async searchIdOrder(id: string) {
    ...
    const product = produto as any;
    const listUser = await this.UserModel.findById({ _id: cliente })
  ...
  async updateOrder(
    ...
    if (!findByIdOrder) throw new NotFoundException();
      ...
      const IdPorduct = updateOrderBody.produto
        ? updateOrderBody.produto
        : findByIdOrder.produto;
      const findByIdProduct = await this.productModel.findById({
        _id: IdPorduct,
      });
      if (!findByIdProduct) throw new NotFoundException();
          ...
          categoriaProduto: {
            [ObjUpdate]: {
              ...
              quantidate:
                ...
                updateOrderBody.quantidadePedido
                  ? findByIdOrder.quantidadePedido +
                    findByIdProduct.categoriaProduto[obj].quantidade
                  : findByIdProduct.categoriaProduto[obj].quantidade,
              imagemProduto:
                findByIdProduct.categoriaProduto[obj].imagemProduto,
        ...
        const updateNewProduct = await this.productModel
          .findByIdAndUpdate(IdPorduct, newProduct)
          .exec();
        if (!updateNewProduct) throw new NotFoundException();
        else {
          ...
          const newOrder = {
            quantidadePedido: updateOrderBody.quantidadePedido
              ? updateOrderBody.quantidadePedido
              : findByIdOrder.quantidadePedido,
            produto: updateOrderBody.produto
              ? updateOrderBody.produto
              : findByIdOrder.produto,
            cliente: updateOrderBody.cliente
              ? updateOrderBody.cliente
              : findByIdOrder.cliente,
            total: updateOrderBody.quantidadePedido
              ? updateOrderBody.quantidadePedido *
                findByIdProduct.categoriaProduto[obj].quantidade
              : findByIdOrder.total,
          };
          const quantidade = updateOrderBody.quantidadePedido as number;
          const newProduct = {
            categoriaProduto: {
              [ObjUpdate]: {
                nome: findByIdProduct.categoriaProduto[ObjUpdate].nome,
                ...
                tamanho: findByIdProduct.categoriaProduto[obj].tamanho,
                preco: findByIdProduct.categoriaProduto[obj].preco,
                quantidate: updateOrderBody.quantidadePedido
                  ? findByIdProduct.categoriaProduto[obj].quantidade -
                    quantidade
                  : findByIdProduct.categoriaProduto[obj].quantidade,
                imagemProduto:
  ...
  async deleteOrder(id: string) {
    const searchIdAndDelete = await this.OrderModel.findByIdAndDelete(
      id
    ).exec();
